    ...
    try:
        ...
        if db_response.data:
            ...
            if selected_lang_code == "ko":
                summary_text = latest_report.get(
                    "summary_ko", "한국어 요약이 없습니다."
                )
            ...
            if split_keyword in summary_text:
                ...
                with st.expander(
                    "🔍 심층 마켓 분석 (Deep Dive Analysis) 전체 보기", expanded=False
                ):
                ...
                if "@" not in sub_email:
                    ...
                    with st.spinner("DB 등록 중..."):
                        ...
                        if result == "success":
                            ...
                            st.success(
                                f"'{sub_email}'님이 구독 리스트에 등록되었습니다."
                            )
                            # 관리자 알림
    ...
    with tab_unsub:
        with st.form(key="unsub_form"):
            unsub_email = st.text_input(
                "구독했던 이메일", placeholder="example@email.com"
            )
